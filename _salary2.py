...
def getSalary(first,last):
    ...
    head = {"User-agent":"Mozilla/5.0 (Windows NT 6.2; WOW64; rv:25.0) Gecko/20100101 Firefox/25.0"}

    r = requests.post(getURL(),data=posts[0],headers=head)
    ms = getMarkStamp(r.text)
    for x in range(1,len(posts)):
        k = posts[x]
        da = k%(ms[0],ms[1])
        r = requests.post(getURL(ms[0]),data=k,headers=head)
        ms = getMarkStamp(r.text)
